...
def run_detection(img):
    ...
    if results[0].boxes is not None:
        for box in results[0].boxes:
            ...
            print(f"🔍 DEBUG: Erkannte Klasse: {class_id}")
            
            if class_id == 0:  # Riffbarsch (basierend auf YOLO Training)
                riffbarsch_count += 1
            elif class_id == 1:  # Taucher
                taucher_count += 1
    
    total_detections = riffbarsch_count + taucher_count
    progress_detect['value'] = 70
    
    print(f"🐠 DEBUG: Riffbarsche: {riffbarsch_count}, Taucher: {taucher_count}")
    ...
    # KORRIGIERTES Balkendiagramm - Klassen-spezifisch
    fig, ax = plt.subplots(figsize=(4,3))
    ...
    ax.set_ylim(0, max(5, max(counts) + 1))  # Mindesthöhe 5 für bessere Sichtbarkeit
